#!/usr/bin/env python3
"""
WOS Manager Cloudways 自動部署工具 (SSH Key 版本)
使用 SSH Key 認證，無需輸入密碼
"""
...
class CloudwaysAutoDeploymentWithKey:
    def __init__(self):
        self.host = "172.105.217.161"
        self.user = "svs2438"
        self.key_path = Path.home() / ".ssh" / "cloudways_rsa"
    ...
    def check_files(self):
        ...
        if not self.key_path.exists():
            self.print_error(f"找不到 SSH 私鑰: {self.key_path}\n請先執行: ssh-keygen -t rsa -b 4096 -f {self.key_path}")
        ...
        self.print_info(f"SSH Key: {self.key_path.name}")
    ...
    def connect_ssh(self):
        """連接到 Cloudways 伺服器"""
        self.print_step(2, 7, "連接到伺服器 (使用 SSH Key)")
        
        try:
            self.print_info(f"伺服器: {self.host}")
            self.print_info(f"用戶: {self.user}")
            self.print_info(f"Key: {self.key_path.name}")
            self.print_info(f"正在連接...")
            ...
            self.print_error(f"認證失敗 - SSH Key 可能未在 Cloudways 設定\n詳見 SSH_KEY_SETUP.md")
        except paramiko.SSHException as e:
            self.print_error(f"SSH 連接失敗: {e}")
        except FileNotFoundError:
            self.print_error(f"SSH Key 文件不存在: {self.key_path}")
    ...
    def execute_remote_command(self, command, description="", show_output=True):
        """執行遠程命令"""
        if description:
    ...
    def execute_deploy(self):
        ...
        deploy_cmd = f"cd {self.remote_path} && bash ./deploy.sh 2>&1"
    ...
    def print_summary(self):
        ...
        print("📊 查看應用狀態:")
        print("   ssh -i /Users/wira/.ssh/cloudways_rsa svs2438@172.105.217.161")
        print("   pm2 status")
        ...
        print("   - 下次部署只需執行本腳本，無需重新設定 Key\n")
    
    def deploy(self):
        """執行完整部署流程"""
        self.print_header("WOS Manager Cloudways 自動部署 (SSH Key)")
...
if __name__ == "__main__":
    deployer = CloudwaysAutoDeploymentWithKey()
    deployer.deploy()
