...
class SubmissionService {
    ...
    static async checkExistingSubmission(userId, dayKey, eventDate) {
        // 取得該使用者所有報名
    ...
    // 管理員更新提交（可更新更多欄位）
    static async adminUpdateSubmission(submissionId, data) {
        const updateData = {};
        if (data.alliance)
            updateData.alliance = data.alliance;
        if (data.playerName)
            updateData.playerName = data.playerName;
        if (data.slots)
            updateData.slotsData = JSON.stringify(data.slots);
        const updated = await prisma.timeslotSubmission.update({
            where: { id: submissionId },
    ...
    // 取得每日提交摘要
    static async getDailySubmissionSummary(reportDate) {
        const submissions = await prisma.timeslotSubmission.findMany({
            where: {
                createdAt: {
        ...
        for (const submission of submissions) {
            ...
            for (const daySlot of Object.values(slots)) {
                const slot = daySlot;
                if (slot?.fireSparkleCount)
                    totalFireSparkle += slot.firㄑnt;
                if (slot?.fireGemCount)
